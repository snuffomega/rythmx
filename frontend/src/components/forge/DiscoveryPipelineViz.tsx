      ...
      <div className="hidden sm:flex px-4 py-3 items-center">
        {steps.map((step, i) => (
          <div key={i} className="flex items-center">
            <div className="flex items-center gap-1.5 py-2 px-3 text-xs text-text-muted">
            ...
            {i < steps.length - 1 && <span className="text-text-faint text-xs px-0.5">{'>'}</span>}
          </div>
        ))}
      </div>
      <div className="flex sm:hidden items-center justify-between px-3 py-3 gap-1">
        {steps.map((step, i) => (
          <div key={i} className="flex items-center gap-1 min-w-0 flex-1">
            <div className="flex flex-col items-center gap-0.5 min-w-0 flex-1 text-text-dim">
            ...
            {i < steps.length - 1 && <span className="text-surface-overlay text-xs flex-shrink-0 mb-3">{'>'}</span>}
          </div>
        ))}
      </div>
    </div>
