...
export function RunHistory({ history, loading }: RunHistoryProps) {
  return (
    <section className="border-t border-border-subtle pt-8">
      <h2 className="text-text-muted text-xs font-semibold uppercase tracking-widest mb-4">Last Run — Release Log</h2>
      {loading ? (
        ...
        <p className="text-text-muted text-sm py-8">No history yet — run New Music to get started</p>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border-subtle">
                <th className="text-left text-text-dim font-medium text-xs uppercase tracking-widest px-0 py-3">Artist</th>
                <th className="text-left text-text-dim font-medium text-xs uppercase tracking-widest px-4 py-3">Album</th>
                <th className="text-left text-text-dim font-medium text-xs uppercase tracking-widest px-4 py-3">Status</th>
                <th className="text-left text-text-dim font-medium text-xs uppercase tracking-widest px-4 py-3 hidden sm:table-cell">Date</th>
              </tr>
            </thead>
            <tbody>
              {history.map((item, i) => (
                <tr key={i} className="border-b border-border-subtle hover:bg-surface-skeleton transition-colors">
