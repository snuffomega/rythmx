...
Resolution order for get_library_reader():
  1. app_settings 'library_backend' (set via Settings UI — persists across restarts)
...
def get_library_reader():
    ...
    Checks app_settings first so the UI can change the backend without a
    ...
    try:
        ...
        saved = rythmx_store.get_setting("library_backend")
        if saved:
            backend = saved
    except Exception:
